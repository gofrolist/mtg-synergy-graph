...
#: Per-qualifier tier definitions for the axis_feeder generator.
#: Each entry maps a qualifier (e.g. ``"attacking"``) to its tier
#: structure: a tuple of ``(tier_name, sql, comment)`` triples ranked
#: by specificity. The generator picks the matching qualifier's tiers
#: and emits one rule per qualifier — mirrors how
#: ``modified_axis_feeder`` and ``counter_axis_feeder`` are organised
#: in ``utility.py``: one rule per axis with multiple tiers, deduped
#: by priority. Add a new qualifier here to extend coverage.
#:
#: SQL note: each query selects ``card_name`` from ``card_ports``.
#: The helper iterates tiers in declaration order and dedupes —
#: anthem (narrowest) wins over peer_scaler (broadest) for the same
#: card.
_AXIS_FEEDER_TIERS: dict[str, tuple[tuple[str, str, str], ...]] = {
    "attacking": (
        (
            "attacking_anthem",
            (
                "SELECT DISTINCT card_name FROM card_ports "
                "WHERE port_type='static' AND event_class='Continuous' "
                "AND raw_line LIKE '%attacking%' "
                "AND (raw_line LIKE '%AddPower%' OR raw_line LIKE '%AddKeyword%')"
            ),
        ...
        (
            ...
            (
                "SELECT DISTINCT card_name FROM card_ports "
                "WHERE port_type='effect' AND event_class='PumpAll' "
                "AND valid_filter LIKE '%attacking%'"
            ),
            "effect.PumpAll on attacking creatures, e.g. Overrun (~73 cards)",
        ),
        (
            "attacking_peer_scaler",
            (
                "SELECT DISTINCT card_name FROM card_ports "
                "WHERE port_type='scales_with' AND event_class='Valid' "
                "AND valid_filter LIKE '%attacking%'"
...
def _gen_axis_feeder(proposal: RuleProposal) -> ScaffoldArtifacts:
    """Generator for the axis_feeder template.

    A commander whose port carries a notable qualifier (``attacking``,
    ``tapped``, etc.) on a non-Self scope wants payoff cards keyed to
    the same axis. Mirrors ``modified_axis_feeder`` /
    ``counter_axis_feeder`` in ``utility.py`` — one rule per qualifier
    with tier-ranked candidate matching.

    Currently supports the ``attacking`` qualifier. Add more by
    extending ``_AXIS_FEEDER_TIERS``. Raises ``ValueError`` for
    unsupported qualifiers — the picker treats this as a skip and
    moves on to the next proposal.
    """
    qualifier = proposal.gap.signature[2]
    if qualifier not in _AXIS_FEEDER_TIERS:
        raise ValueError(
            f"axis_feeder generator has no tier definition for qualifier "
            f"{qualifier!r}; add an entry to _AXIS_FEEDER_TIERS"
        )
    rule_id = f"{_sanitize_for_identifier(qualifier)}_axis_feeder"
    tiers = _AXIS_FEEDER_TIERS[qualifier]

    # Build the helper's tier search block as Python source.
    tier_blocks = []
    for tier_name, sql, comment in tiers:
        tier_blocks.append(
            f"    # {comment}\n"
            f"    {tier_name}_set: set[str] = {{\n"
            f'        row["card_name"]\n'
            f"        for row in conn.execute(\n"
            f"            {sql!r}\n"
            f"        )\n"
            f"    }}"
        )
    ...
    tier_priority_src = ",\n".join(f'        ("{name}", {name}_set)' for name, _, _ in tiers)

    helper_src = f'''"""AUTO-GENERATED rule: {rule_id}.

Generated by scripts/scaffold_rule.py from template 'axis_feeder' on
qualifier {qualifier!r}. Surfaces tier-ranked payoff cards for any
...
Auditor context at generation time:
- Reach: {proposal.gap.commanders} commanders carrying this signature
- Currently uncovered: {proposal.gap.commanders - proposal.gap.activations}
"""
...
def _{rule_id}_gate(port: PortRow) -> bool:
    ...
    - OppCtrl scope (defensive utilities like Catti-brie's anti-attacker
      ...
      pump)
    """
    if (port.get("port_type") or "").strip() not in ("trigger", "scales_with", "static", "effect", "cost", "replacement"):
        return False
...
class TestGate:
    ...
    def test_unrelated_port_type_skips(self):
        assert not _{rule_id}_gate(_port(port_type="keyword", event_class="Vigilance", valid_filter=""))
...
'''
    ...
    return ScaffoldArtifacts(
        ...
        registry_gate_line=f'    RuleGate("{rule_id}", _{rule_id}_gate),',
        scorer_bucket_entry=f'    "{rule_id}": "port_match",',
        scorer_multiplier_entry=f'    # AUTO-GENERATED axis-feeder ({qualifier}) — IDF handles weighting\n    "{rule_id}": 2.5,',
        bucket="port_match",
        multiplier=2.5,
    )
