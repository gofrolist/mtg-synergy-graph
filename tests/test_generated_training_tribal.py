"""AUTO-GENERATED tests for rule: training_tribal."""
...
from mtg_synergy_graph.complement_rules.generated.training_tribal import (
    _find_training_tribal,
    _training_tribal_gate,
)
...
class TestTrainingGate:
    def test_non_keyword_port_skips(self):
        assert not _training_tribal_gate(_port(port_type="trigger", event_class="Training"))

    def test_other_keyword_skips(self):
        assert not _training_tribal_gate(_port(port_type="keyword", event_class="Flying"))

    def test_target_keyword_matches(self):
        assert _training_tribal_gate(_port(port_type="keyword", event_class="Training"))


class TestFindTraining:
    def _cmdr_ports(self):
        return [_port(port_type="keyword", event_class="Training")]

    def test_no_keyword_port_returns_empty(self, conn):
        assert _find_training_tribal(conn, [_port(port_type="trigger", event_class="Attacks")], set()) == []

    def test_finds_other_cards_with_keyword(self, conn):
        _add_keyword(conn, "Partner Card A", "Training")
        _add_keyword(conn, "Partner Card B", "Training")
        results = _find_training_tribal(conn, self._cmdr_ports(), set())
    ...
    def test_excludes_commander(self, conn):
        _add_keyword(conn, "Self Commander", "Training")
        results = _find_training_tribal(conn, self._cmdr_ports(), {"Self Commander"})
        names = {r.candidate for r in results}
        assert "Self Commander" not in names

    def test_rule_id(self, conn):
        _add_keyword(conn, "Partner Card", "Training")
        results = _find_training_tribal(conn, self._cmdr_ports(), set())
        assert all(r.rule_id == "training_tribal" for r in results)
