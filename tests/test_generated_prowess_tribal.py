"""AUTO-GENERATED tests for rule: prowess_tribal."""
...
from mtg_synergy_graph.complement_rules.generated.prowess_tribal import (
    _find_prowess_tribal,
    _prowess_tribal_gate,
)
...
class TestProwessGate:
    def test_non_keyword_port_skips(self):
        assert not _prowess_tribal_gate(_port(port_type="trigger", event_class="Prowess"))

    def test_other_keyword_skips(self):
        assert not _prowess_tribal_gate(_port(port_type="keyword", event_class="Flying"))

    def test_target_keyword_matches(self):
        assert _prowess_tribal_gate(_port(port_type="keyword", event_class="Prowess"))


class TestFindProwess:
    def _cmdr_ports(self):
        return [_port(port_type="keyword", event_class="Prowess")]

    def test_no_keyword_port_returns_empty(self, conn):
        assert _find_prowess_tribal(conn, [_port(port_type="trigger", event_class="Attacks")], set()) == []

    def test_finds_other_cards_with_keyword(self, conn):
        _add_keyword(conn, "Partner Card A", "Prowess")
        _add_keyword(conn, "Partner Card B", "Prowess")
        results = _find_prowess_tribal(conn, self._cmdr_ports(), set())
    ...
    def test_excludes_commander(self, conn):
        _add_keyword(conn, "Self Commander", "Prowess")
        results = _find_prowess_tribal(conn, self._cmdr_ports(), {"Self Commander"})
        names = {r.candidate for r in results}
        assert "Self Commander" not in names

    def test_rule_id(self, conn):
        _add_keyword(conn, "Partner Card", "Prowess")
        results = _find_prowess_tribal(conn, self._cmdr_ports(), set())
        assert all(r.rule_id == "prowess_tribal" for r in results)
