...
class TestFindTokenSacChain:
    """Tests for _find_token_sac_chain."""

    def test_no_sac_trigger_returns_empty(self, conn: sqlite3.Connection) -> None:
        """Commander without Sacrificed trigger produces no results."""
        ports = [_port("Cmdr", "trigger", "ChangesZone")]
        assert _find_token_sac_chain(conn, ports, {"Cmdr"}) == []

    def test_non_trigger_ports_skipped(self, conn: sqlite3.Connection) -> None:
        """Non-trigger ports are skipped when scanning for Sacrificed -> line 351-352."""
        ports = [
            _port("Cmdr", "effect", "Sacrificed"),
            _port("Cmdr", "trigger", "Sacrificed"),
        ]
        _insert_port(
            conn,
            "Treasure Maker",
            "effect",
            "Token",
            raw_line="'TokenScript': 'c_a_treasure_sac'",
        )
        results = _find_token_sac_chain(conn, ports, {"Cmdr"})
        assert len(results) == 1

    def test_korvold_finds_treasure_producers(self, conn: sqlite3.Connection) -> None:
        """Korvold (Sacrificed trigger) finds Treasure token producers -> line 373, 377+."""
        ports = [_port("Korvold", "trigger", "Sacrificed")]

        # Card producing treasure tokens
        _insert_port(
            conn,
            "Pitiless Plunderer",
            "effect",
        ...
        assert len(results) == 1
        assert results[0].candidate == "Pitiless Plunderer"
        assert results[0].rule_id == "token_sac_chain"
        assert results[0].cmdr_event == "Sacrificed"
        assert "treasure" in results[0].cand_event.lower()

    def test_finds_food_producers(self, conn: sqlite3.Connection) -> None:
        """Food token producers matched."""
        ports = [_port("Korvold", "trigger", "Sacrificed")]
        ...
        _insert_port(
            ...
            "effect",
            "Token",
            raw_line="'TokenScript': 'c_a_food_sac'",
        )

        results = _find_token_sac_chain(conn, ports, {"Korvold"})
        assert len(results) == 1
        assert results[0].candidate == "Gilded Goose"

    def test_finds_clue_producers(self, conn: sqlite3.Connection) -> None:
        """Clue token producers matched."""
        ports = [_port("Korvold", "trigger", "Sacrificed")]
        ...
        _insert_port(
            ...
            "effect",
            "Token",
            raw_line="'TokenScript': 'c_a_clue_sac'",
        )

        results = _find_token_sac_chain(conn, ports, {"Korvold"})
        assert len(results) == 1

    def test_finds_blood_producers(self, conn: sqlite3.Connection) -> None:
        """Blood token producers matched."""
        ports = [_port("Korvold", "trigger", "Sacrificed")]
        ...
        _insert_port(
            ...
            "effect",
            "Token",
            raw_line="'TokenScript': 'c_a_blood_sac'",
        )

        results = _find_token_sac_chain(conn, ports, {"Korvold"})
        assert len(results) == 1

    def test_non_sac_token_not_matched(self, conn: sqlite3.Connection) -> None:
        """Token producer without sac-pattern (e.g. soldier) not matched -> line 377."""
        ports = [_port("Korvold", "trigger", "Sacrificed")]
        ...
        _insert_port(
            ...
            "effect",
            "Token",
            raw_line="'TokenScript': 'w_1_1_soldier'",
        )
    ...
    def test_commander_excluded(self, conn: sqlite3.Connection) -> None:
        """Commander's own name excluded from results -> line 372-373."""
        ports = [_port("Korvold", "trigger", "Sacrificed")]
        ...
        results = _find_token_sac_chain(conn, ports, {"Korvold"})
        candidates = {r.candidate for r in results}
        assert "Korvold" not in candidates
        assert "Other Card" in candidates

    def test_no_token_script_in_raw_line_skipped(self, conn: sqlite3.Connection) -> None:
        """Token effect without TokenScript in raw_line is skipped -> line 375-377."""
        ports = [_port("Korvold", "trigger", "Sacrificed")]
        ...
        _insert_port(
            ...
            "effect",
            "Token",
            raw_line="some other raw line without script",
        )

        results = _find_token_sac_chain(conn, ports, {"Korvold"})
        assert len(results) == 0

    def test_deduplication(self, conn: sqlite3.Connection) -> None:
        """Same card with multiple token effects is only returned once."""
        ports = [_port("Korvold", "trigger", "Sacrificed")]
