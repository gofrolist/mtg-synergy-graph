"""AUTO-GENERATED tests for rule: choose_tribal."""
...
from mtg_synergy_graph.complement_rules.generated.choose_tribal import (
    _choose_tribal_gate,
    _find_choose_tribal,
)
...
class TestChooseGate:
    def test_non_keyword_port_skips(self):
        assert not _choose_tribal_gate(_port(port_type="trigger", event_class="Choose"))

    def test_other_keyword_skips(self):
        assert not _choose_tribal_gate(_port(port_type="keyword", event_class="Flying"))

    def test_target_keyword_matches(self):
        assert _choose_tribal_gate(_port(port_type="keyword", event_class="Choose"))


class TestFindChoose:
    def _cmdr_ports(self):
        return [_port(port_type="keyword", event_class="Choose")]

    def test_no_keyword_port_returns_empty(self, conn):
        assert _find_choose_tribal(conn, [_port(port_type="trigger", event_class="Attacks")], set()) == []

    def test_finds_other_cards_with_keyword(self, conn):
        _add_keyword(conn, "Partner Card A", "Choose")
        _add_keyword(conn, "Partner Card B", "Choose")
        results = _find_choose_tribal(conn, self._cmdr_ports(), set())
    ...
    def test_excludes_commander(self, conn):
        _add_keyword(conn, "Self Commander", "Choose")
        results = _find_choose_tribal(conn, self._cmdr_ports(), {"Self Commander"})
        names = {r.candidate for r in results}
        assert "Self Commander" not in names

    def test_rule_id(self, conn):
        _add_keyword(conn, "Partner Card", "Choose")
        results = _find_choose_tribal(conn, self._cmdr_ports(), set())
        assert all(r.rule_id == "choose_tribal" for r in results)
