"""AUTO-GENERATED tests for rule: ward_1_tribal."""
...
from mtg_synergy_graph.complement_rules.generated.ward_1_tribal import (
    _find_ward_1_tribal,
    _ward_1_tribal_gate,
)
...
class TestWard1Gate:
    def test_non_keyword_port_skips(self):
        assert not _ward_1_tribal_gate(_port(port_type="trigger", event_class="Ward:1"))

    def test_other_keyword_skips(self):
        assert not _ward_1_tribal_gate(_port(port_type="keyword", event_class="Flying"))

    def test_target_keyword_matches(self):
        assert _ward_1_tribal_gate(_port(port_type="keyword", event_class="Ward:1"))


class TestFindWard1:
    def _cmdr_ports(self):
        return [_port(port_type="keyword", event_class="Ward:1")]

    def test_no_keyword_port_returns_empty(self, conn):
        assert _find_ward_1_tribal(conn, [_port(port_type="trigger", event_class="Attacks")], set()) == []

    def test_finds_other_cards_with_keyword(self, conn):
        _add_keyword(conn, "Partner Card A", "Ward:1")
        _add_keyword(conn, "Partner Card B", "Ward:1")
        results = _find_ward_1_tribal(conn, self._cmdr_ports(), set())
    ...
    def test_excludes_commander(self, conn):
        _add_keyword(conn, "Self Commander", "Ward:1")
        results = _find_ward_1_tribal(conn, self._cmdr_ports(), {"Self Commander"})
        names = {r.candidate for r in results}
        assert "Self Commander" not in names

    def test_rule_id(self, conn):
        _add_keyword(conn, "Partner Card", "Ward:1")
        results = _find_ward_1_tribal(conn, self._cmdr_ports(), set())
        assert all(r.rule_id == "ward_1_tribal" for r in results)
