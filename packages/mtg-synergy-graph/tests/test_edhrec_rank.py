    ...
    for cmc, names in by_cmc.items():
        ...
        for i in range(len(names) - 1):
            assert ranks[i] <= ranks[i + 1], (
                ...
                f"{names[i]} (rank {ranks[i]}) precedes "
                f"{names[i + 1]} (rank {ranks[i + 1]})"
            )
...
def test_kyler_tied_cluster_is_not_purely_alphabetical():
    ...
    assert bucket != sorted(bucket), (
        "Kyler's tied 18.0 cluster is still in pure alphabetical "
        "order — the edhrec_rank tiebreaker is not taking effect."
    )
