...
SEMANTIC_BRIDGES = {
    ...
    ("tokens-creature", "etb-value"): 0.85,
    ("tokens-creature", "etb-tokens"): 0.9,
    ("token-doubling", "etb-value"): 0.6,
    ("token-doubling", "etb-tokens"): 0.7,

    # Creature pump ↔ creature power
    ("pump-anthem", "creature-power"): 0.85,
    ("pump-lord", "creature-power"): 0.85,
    ...
    ("evasion-unblockable", "attack-events"): 0.6,
    ("evasion-flying", "attack-events"): 0.5,
    ("haste-grant", "attack-events"): 0.6,
    ("combat-enabler", "attack-events"): 0.7,
    ("combat-enabler", "combat-attack"): 0.7,
    ("combat-trigger", "combat-attack"): 0.85,
    ...
    # Token generation → board & token events
    ("tokens-creature", "board-go-wide"): 0.8,
    ("tokens-creature", "board-tokens"): 0.95,
    ("tokens-creature", "token-events"): 0.85,
    ...
    ("board-protection", "board-generic"): 0.4,
    ...
    ("tokens-creature", "sacrifice-events"): 0.85,
    ("tokens-creature", "creature-death"): 0.65,

    # Sacrifice payoff wants creature death / sacrifice events
    ("sacrifice-payoff", "creature-death"): 0.8,
    ("sacrifice-payoff", "etb-value"): 0.4,
    ...
    ("tokens-tribal", "goblin-tribal"): 0.6,
    ("cost-reduction", "goblin-tribal"): 0.5,
    ("tutor", "goblin-tribal"): 0.6,

    # ETB payoff wants creature board / token events
    ("etb-payoff", "board-go-wide"): 0.5,
    ("etb-payoff", "token-events"): 0.6,

    # Damage dealing provides what ETB wants
    ("damage-dealing", "etb-value"): 0.4,
    ...
    ("pump-anthem", "attack-events"): 0.55,
    ("pump-combat", "attack-events"): 0.55,

    # Counter-related provides → counter-placement-events
    # (evasion/pump payoffs that care about counters — removed weak evasion-grant bridge)

    # Artifact/enchantment removal benefits from creature ETB (Aura Shards pattern)
    ("artifact-enchantment-removal", "etb-value"): 0.4,

    # Counter placement → board (placing counters means having creatures)
    ("counter-placement", "board-generic"): 0.4,
    ("board-wide-counter-placement", "board-go-wide"): 0.5,
    ...
    ("tokens-creature", "equip-target"): 0.65,
    ...
    ("evasion-unblockable", "combat-attackers"): 0.8,
    ("evasion-flying", "combat-attackers"): 0.6,
    ("pump-anthem", "combat-attackers"): 0.7,
    ("pump-combat", "combat-attackers"): 0.7,
    ("tokens-creature", "combat-attackers"): 0.65,  # More bodies to attack with
    ...
    ("tokens-creature", "creature-copy-target"): 0.5,

    # General board bridges (kept but lower weight since generic)
    ("tokens-creature", "board-generic"): 0.5,
    ("graveyard-recursion", "board-generic"): 0.4,
    ("board-protection", "board-go-wide"): 0.5,

    # ── Bidirectional combat/mana loops ──
    ("untap", "combat-attack"): 0.5,           # Untap after combat enables re-use
    ("combat-enabler", "combat-attackers"): 0.8,

    # ── Counter/ETB cycles ──
    ("creature-etb-payoff", "tokens-creature"): 0.7,  # ETB payoff benefits from tokens entering
    ...
    ("triggered-abilities", "etb-value"): 0.4,  # Triggered abilities often involve ETB
    ...
    ("mana-acceleration", "etb-value"): 0.4,     # Mana enables casting creatures (ETB)
    ...
    ("untap", "etb-value"): 0.4,                  # Untap bounce/blink sources
    ...
    ("creature-type-flexibility", "etb-value"): 0.5,
    ("tribal-enabler", "etb-value"): 0.4,
    ...
    ("pump-anthem", "creature-etb-payoff"): 0.5,     # Pump + ETB payoff = growing board
    ("evasion-unblockable", "combat-attack"): 0.65,  # Evasion enables combat (Blighted Agent + Inkmoth)
    ("evasion-flying", "combat-attack"): 0.55,
    ("infect", "combat-attack"): 0.6,                # Infect creatures want combat
    ...
    ("life-drain", "etb-value"): 0.4,                 # Drain on ETB (Blood Artist)

    # ── Final push: 3-5 combo bridges ──
    ("mana-acceleration", "creature-death"): 0.3,     # Mana + death triggers (Ashnod's Altar patterns)
    # Removed weak evasion-grant → mana-needs (0.3)
    ("combat-enabler", "mana-needs"): 0.3,
    ("combat-enabler", "etb-value"): 0.4,             # Combat enablers trigger ETB
    ("board-pressure", "equip-target"): 0.4,          # Pressure cards benefit from equipment
    ("board-protection", "etb-value"): 0.4,           # Protected creatures keep triggering
    ...
    ("evasion-flying", "triggered-abilities"): 0.4,   # Evasion enables combat triggers
    ...
    ("bounce-utility", "etb-value"): 0.7,            # Bounce + recast = ETB again
    ...
    ("board-wipe", "etb-value"): 0.4,                # Wipe + recursion = re-ETB
    ...
    ("evasion-flying", "life-gain-events"): 0.4,  # Evasion + lifelink = life gain

    # ── Extra turn / combat bridges ──
    ("extra-turn", "spell-casting"): 0.6,       # Extra turns let you cast more spells
    ("extra-turn", "combat-attack"): 0.7,        # Extra turns = extra combat steps
    ("extra-turn", "card-draw-events"): 0.6,     # Extra turns = extra draw steps
    ("extra-turn", "etb-value"): 0.5,            # Extra turns = more creatures played

    # ── Damage bridges ──
    ("damage-dealing", "combat-damage"): 0.6,    # Damage dealers benefit from combat
    ...
    ("graveyard-recursion", "etb-value"): 0.7,   # Recurring creatures trigger ETB
    ...
    ("ability-copying", "etb-value"): 0.6,         # Copy ETB abilities
    ("spell-copying", "etb-value"): 0.5,           # Copy creature spells = more ETBs
    ("flash-grant", "etb-value"): 0.5,             # Flash enables surprise ETBs
    ("tokens-creature", "spell-casting"): 0.4,     # Token makers that trigger on cast
    ...
    ("tokens-creature", "sacrifice-outlet"): 0.65,
    ...
    # ── Key sub-tag bridges (high-weight, specific) ──
    ("tokens-tribal", "etb-tribal"): 0.95,
    ("pump-lord", "board-tribal"): 0.95,
    ("pump-anthem", "board-go-wide"): 0.9,
    ("tokens-artifact", "artifact-presence"): 0.8,
    ("evasion-menace", "combat-attack"): 0.6,
}
